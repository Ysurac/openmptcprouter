...
ImageInfo = namedtuple("ProgInfo", "name filename type primary")
...
class MIBIB(object):
    ...
    def __read_parts(self, part_fp):
        ...
        for i in range(mtable.numparts):
            ...
            part_name = mentry.name.strip(b'\x00').decode()
...
class Pack(object):
    ...
    def __gen_flash_script(self, info, script, flinfo):
        ...
        for section in info.sections():
            ...
            try:
                primary = info.get(section, "primary")
            except ConfigParserError as e:
                primary = "0"
            ...
            if ((self.flinfo.type == 'nand' or self.flinfo.type == 'emmc') and (size != img_size)):
                ...
                # Append padding bytes: 0xFF for NAND, 0x00 for EMMC
                pad_byte = b'\xff' if self.flinfo.type == 'nand' else b'\x00'
                try:
    ...
    def __mkimage(self, images):
        ...
        image_data = []
        for (section, fname, imtype, primary) in images:
            if primary != "0":
               continue
    ...
    def main_bconf(self, flash_type, images_dname, out_fname, brdconfig):
        ...
        images.insert(0, ImageInfo("script", "flash.scr", "script","0"))
