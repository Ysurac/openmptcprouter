...
add_sub:
		...
		if (!strncmp(part->name, "0:MIBIB", FDT_SMEM_MAX_PART_NAME_LENGTH) ||
			!strncmp(part->name, "0:SBL1", FDT_SMEM_MAX_PART_NAME_LENGTH) ||
			!strncmp(part->name, "0:QSEE", FDT_SMEM_MAX_PART_NAME_LENGTH) ||
			!strncmp(part->name, "0:CDT", FDT_SMEM_MAX_PART_NAME_LENGTH) ||
			!strncmp(part->name, "0:CONFIG", FDT_SMEM_MAX_PART_NAME_LENGTH) ||
			!strncmp(part->name, "0:APPSBL", FDT_SMEM_MAX_PART_NAME_LENGTH) ||
			part->mask_flags & 1) {
add_ro:
			debug("Adding read only property for (%s)", part->name);
			ret = fdt_setprop(blob, newoff, "read-only", NULL, 0);
			if (ret == -FDT_ERR_NOSPACE) {
				ret = fdt_increase_size(blob, 512);
				if (!ret)
					goto add_ro;
