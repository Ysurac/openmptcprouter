 ...
 */
#define VERSION_NUMBER "V1.2.0.21"
...
static int qmap_register_device(sQmiWwanQmap * pDev, u8 offset_id)
{
	...
	rmnet_nss_dereference(nss_cb);
#ifdef QUECTEL_BRIDGE_MODE
	if (pDev->bridge_mode & BIT(offset_id))
		nss_cb = NULL;
#endif
	...
	qmap_net = alloc_netdev(sizeof(struct qmap_priv), name,
				NET_NAME_UNKNOWN,
				need_rawip ? rmnet_usb_rawip_setup : rmnet_usb_ether_setup);
#else
	qmap_net = alloc_netdev(sizeof(struct qmap_priv), name,
				need_rawip ? rmnet_usb_rawip_setup : rmnet_usb_ether_setup);
#endif
...
static void qmap_unregister_device(sQmiWwanQmap * pDev, u8 offset_id) {
	...
	if (qmap_net != NULL && qmap_net != pDev->mpNetDev->net) {
		...
		rmnet_nss_dereference(nss_cb);
		rcu_read_lock();
		rx_handler = rcu_dereference(qmap_net->rx_handler);
		rcu_read_unlock();
		if (nss_cb && rx_handler == qca_nss_rx_handler) {
			rtnl_lock();
...
static int qmap_qmi_wwan_rx_fixup(struct usbnet *dev, struct sk_buff *skb_in)
{
	...
	while ((qmap_skb = skb_dequeue (&skb_chain))) {
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!status)
	{
		...
		if (dev->driver_info->flags & FLAG_NOARP)
		{
			...
			int lte_a = (idProduct == 0x0306 || idProduct == 0x0512 || idProduct == 0x0620 || idProduct == 0x0800 || idProduct == 0x90db|| idProduct == 0x90d5 || idProduct == 0x4d22 );
			...
			if (pQmapDev->qmap_mode) {
				...
				else if (idProduct == 0x0800  ){
					pQmapDev->use_rmnet_usb = 1; //benefit for ul data agg	
					pQmapDev->rmnet_info.iface_id = 4;
					}
				else if (idProduct == 0x4d22 ){
					pQmapDev->use_rmnet_usb = 1; //benefit for ul data agg	
					pQmapDev->rmnet_info.iface_id = 5;
					}
				else if ( idProduct == 0x90db ){
					pQmapDev->use_rmnet_usb = 1; //benefit for ul data agg	
					pQmapDev->rmnet_info.iface_id = 5;
					}	
...
static const struct usb_device_id products[] = {
	...
	{ QMI_FIXED_RAWIP_INTF(0x2C7C, 0x030A, 6, mdm9x07) },  /* Quectel EG05 */
