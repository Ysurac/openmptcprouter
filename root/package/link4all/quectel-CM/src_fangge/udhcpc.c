...
static void* udhcpc_thread_function(void*  arg) {
    ...
    if (profile->IPType == 0x04)
    {
       ...
       // }
        ...
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "%s", "getip -i lte0 -s /etc/dhcp.bound -p /var/dhcp.pid");
    }
    else
    {
    /*
...
void udhcpc_start(PROFILE_T *profile) {
    ...
    /* start 前先调用stop，确保原来的dhcpc进程退出*/
    if(do_kill_first)udhcpc_stop(profile);
...
static int getDhcpcPid(void)
{
	int pid = 0;
	char str[64] = {0};
	FILE* fs = NULL;

	fs = fopen("/var/dhcp.pid", "r");
	if ( fs != NULL ) 
	{
		if ( fgets(str, 64, fs) > 0 )
		{
			pid = atoi(str);
		}
		fclose(fs);
...
void udhcpc_stop(PROFILE_T *profile) {
    const char *ifname = profile->usbnet_adapter;
    do_kill_first = 0;
	system("3g-mngr ipdown");
	sleep(1);

#ifdef ANDROID
#else
    pid_t pid = getDhcpcPid();
    ...
    if(pid > 0){
        ...
        do {
            usleep(100*1000);
        } while((kill_time--) > 0 && udhcpc_pid != 0); //wait udhcpc quit
    }

    unlink("/var/dhcp.pid");    //if kill is ok, just remove the file
    #endif
#endif    
    ifc_init();
