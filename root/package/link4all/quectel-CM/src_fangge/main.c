...
#define POLL_DATA_CALL_STATE_SECONDS 15 
...
#define MEIGE_CM_DEBUG

#ifdef  MEIGE_CM_DEBUG
#define usbnet_link_change(link, profile) usbnet_link_changeEx(link, profile, __FUNCTION__, __LINE__)

static void usbnet_link_changeEx(int link, PROFILE_T *profile, char *fun, int li) {
    printf("%s %d, link(%d -> %d)\r\n", fun, li, s_link, link);
#else
...
#define KVERSION(j,n,p)    ((j)*1000000 + (n)*1000 + (p))
    ...
    while ((ent = readdir(pDir)) != NULL) {
        if ((strncmp(ent->d_name, "cdc-wdm", strlen("cdc-wdm")) == 0) || (strncmp(ent->d_name, "qcqmi0", strlen("qcqmi0")) == 0)) {
            char net_path[512];
            ...
            *pp_qmichannel = (char *)malloc(32);
            ...
            {
               // sprintf(net_path, "/sys/class/net/eth%s", "0");


                #ifndef ANDROID
		if (kernel_version >= KVERSION( 2,6,39 ))
                    sprintf(net_path, "/sys/class/net/eth%d", 0);
		else
		    sprintf(net_path, "/sys/class/net/usb%d", 0);
                #else
                if (access(net_path, R_OK) && errno == ENOENT)
                   //sprintf(net_path, "/sys/class/net/eth%s", &ent->d_name[strlen("qcqmi")]);
                   sprintf(net_path, "/sys/class/net/usb%s", "0");
                #endif
            }
...
int meige_proc(int argc, char *argv[])
#else
...
#endif
{
    ...
    {
        FILE *meige_fs = NULL;
	    if((meige_fs=fopen("/var/meige_proc", "w+")) != NULL){
            fprintf(meige_fs, "%d\n", getpid());
            fclose(meige_fs);
        }
    }
...
#define has_more_argv() ((opt < argc) && (argv[opt][0] != '-'))
    ...
    dbg_time("meige modem init start");
...
__main_loop:
    while (!profile.qmichannel)
    {
        ...
        if (main_loop)
        {
            ...
            dbg_time("Wait for meige modules connect");
        ...
        dbg_time("Cannot find qmichannel(%s) usbnet_adapter(%s) for meige modules", profile.qmichannel, profile.usbnet_adapter);
        return -ENODEV;
    }
...
#if 0 //for test only, make fd > 255
{
    ...
    if (!strncmp(profile.qmichannel, "/dev/qcqmi0", strlen("/dev/qcqmi0")))
    {
...
__main_quit:
    ...
    if(access("/var/meige_proc", F_OK) == 0){
        unlink("/var/meige_proc");
    }
