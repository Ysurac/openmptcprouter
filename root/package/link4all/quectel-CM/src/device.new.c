...
#define USB_INTERFACE_IS_QMI(_Class, _SubClass) (_Class == 0xff && _SubClass == 0xff)
#define USB_INTERFACE_IS_MBIM(_Class, _SubClass) (_Class == USB_CLASS_COMM && _SubClass == USB_CDC_SUBCLASS_MBIM)
...
static int detect_path_cdc_wdm_or_qcqmi(char *path, size_t len)
{
    ...
    if (!access(path, R_OK))
    {
        ...
        path[offset] = '\0';
        // should we check endpint??? why bNumEndpoints == 3???, for unisoc mbim bNumEndpoints = 1
        ...
        if (USB_INTERFACE_IS_QMI(bInterfaceClass, bInterfaceSubClass))
            ; //qmi_wwan / GobiNet
        else if (USB_INTERFACE_IS_MBIM(bInterfaceClass, bInterfaceSubClass))
            ; //cdc_mbim
        else
            return -1;
...
BOOL qmidevice_detect(char *qmichannel, char *usbnet_adapter, unsigned bufsize, int *pbusnum, int *pdevnum) {
    ...
    while ((ent = readdir(pDir)) != NULL)  {
        ...
        if (idVendor != 0x05c6 && idVendor != 0x2c7c && idVendor != 0x1e0e && idVendor != 0x2df3)
            continue;
        ...
        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.3/net", rootdir, ent->d_name);
        dir_get_child(pl->path, netcard, sizeof(netcard));
        if (netcard[0] == '\0') {
            if (idVendor == 0x05c6)
            {
                if(idProduct == 0x90db){
                    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.2/net", rootdir, ent->d_name); //for 90db 
                    dir_get_child(pl->path, netcard, sizeof(netcard));
                }
                if(idProduct == 0xf601){
                    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.5/net", rootdir, ent->d_name); //for slm730
                    dir_get_child(pl->path, netcard, sizeof(netcard));
                }
            }
            else if(idVendor == 0x1e0e)
            {
                if (idProduct == 0x901e)
                {
                    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.2/net", rootdir, ent->d_name); //for 901e 
                    dir_get_child(pl->path, netcard, sizeof(netcard));
                }
                else
                {
                    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.5/net", rootdir, ent->d_name); //for 9001 
                    dir_get_child(pl->path, netcard, sizeof(netcard));
                }
            }
            else if(idVendor == 0x2c7c){
                if(idProduct == 0x0125||idProduct == 0x0121||idProduct == 0x00306 ||idProduct == 0x030a ){
                    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.4/net", rootdir, ent->d_name); //for EC20 EC21/EC25 EP06
                    printf("ec20 em05 usb path is :%s\n",pl->path);
                    dir_get_child(pl->path, netcard, sizeof(netcard));
                }
            }
            else
            {
                snprintf(pl->path, sizeof(pl->path), "%s/%s:1.4/net", rootdir, ent->d_name); //for 0800 
                dir_get_child(pl->path, netcard, sizeof(netcard));
            }
        }
...
int ql_capture_usbmon_log(PROFILE_T *profile, const char *log_path)
{
    ...
    sprintf(usbmon_path, "cat /sys/kernel/debug/usb/devices >> %s", log_path);
    system(usbmon_path);
