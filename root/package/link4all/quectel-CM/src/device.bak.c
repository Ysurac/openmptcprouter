...
BOOL qmidevice_detect(char *qmichannel, char *usbnet_adapter, unsigned bufsize, int *pbusnum, int *pdevnum) {
    ...
    while ((ent = readdir(pDir)) != NULL)  {
        ...
        bNumInterfaces += 8;
        while (bNumInterfaces >= 0) { //RG500U's MBIM is at inteface 0
            snprintf(pl->path, sizeof(pl->path), "%s/%s:1.%d/net", rootdir, ent->d_name, bNumInterfaces-1);
