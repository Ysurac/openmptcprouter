...
#include <stdio.h>
#include <uci.h>
int read_config(const char *path,const char *package,const char *section,const char *option,char *value)
{
	...
	if(NULL == c)
	{
		printf("uci_alloc_context error !\n");
		goto cleanup;
	}

	if(NULL != path)
	{
		if(UCI_OK != uci_set_confdir( c, path)) //设置文件路径
		{
			printf("uci_set_confdir error !\n");
	...
	if(NULL == package)
	{
		sprintf(buf,"lte_info.%s.%s",section,option);

	}
	else
	{
		sprintf(buf,"%s.%s.%s",package,section,option); //设置读取的节点
	}
...
char * get_iface(const char *path,const char *option,char *result)
{
    strcpy(result,"N");
...
// static void  get_iface(const char *ifname,char *result)
// {
//     FILE *fp; 
//     char cmd[128]; 
//     sprintf(cmd,"/sbin/getiface.sh %s",ifname);
//     fp=popen(cmd, "r"); 
...
void update_ipv4_address(const char *ifname, const char *ip, const char *gw, unsigned prefix)
{
    ...
    if (!access("/sbin/ip", X_OK)) {
        ...
        get_iface("/etc/config/network",ifname,buf2);
        read_config("/etc/config/","network",buf2, "metric",metric);
        if(metric != NULL){
            printf("ifname is %s\n",ifname);
        ...
        ql_system(shell_cmd);
        // snprintf(shell_cmd, sizeof(shell_cmd), "cp /tmp/resolv.conf /tmp/resolv.conf.auto");
        // ql_system(shell_cmd);
    } else {
        ...
        char buf2[128];
        bzero(buf2,sizeof(buf2));
        // get_iface(ifname,buf2);
        get_iface("/etc/config/network",ifname,buf2);
        printf("ifname is %s,buf2 value is: %s\n",ifname,buf2);
        snprintf(shell_cmd, sizeof(shell_cmd), "ifup %s",buf2);
        printf("openwrt system run \"ifup %s\" \n",buf2);
        ql_system(shell_cmd);
    }
}
...
/* Do DHCP using busybox tools */
    {
        ...
        get_iface("/etc/config/network",ifname,buf2);
        printf("ifname is %s,buf2 value is: %s\n",ifname,buf2);
        snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "ifup %s",buf2);
        printf("openwrt system run \"ifup %s\" \n",buf2);
#endif
