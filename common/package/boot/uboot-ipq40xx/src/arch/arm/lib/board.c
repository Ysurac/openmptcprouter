...
static int shift_register_set_pins_low(void)
{
#define PIN_DATA  3
#define PIN_LATCH 2
#define PIN_CLOCK 1

	gpio_set_value(PIN_DATA, 0); // set QA-QH low
	gpio_set_value(PIN_LATCH, 0);
	gpio_set_value(PIN_CLOCK, 0);

	gpio_tlmm_config(PIN_DATA, 0, 0, 0, 0, 1, 0, 0, 0);
	gpio_tlmm_config(PIN_LATCH, 0, 0, 0, 0, 1, 0, 0, 0);
	gpio_tlmm_config(PIN_CLOCK, 0, 0, 0, 0, 1, 0, 0, 0);

	mdelay(1);

	for (unsigned c = 24; c; c--) { // 3x 74HC595
...
static int reset_peripherials(void) {
	#define PIN_SYS_RST  62

	gpio_tlmm_config(PIN_SYS_RST, 0, 0, 0, 0, 1, 0, 0, 0);
	gpio_set_value(PIN_SYS_RST, 0); 	
	mdelay(50);
	gpio_set_value(PIN_SYS_RST, 1); 
